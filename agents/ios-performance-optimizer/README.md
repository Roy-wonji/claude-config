...
### 케이스 1: Attendance iOS 2024 프로젝트
```bash
npm start "전체 프로젝트 최적화해줘"
```

**자동 개선 결과:**
- 🔧 QR 스캐너 메모리 누수 자동 수정
- 🎨 AttendanceCard 렌더링 최적화 적용
- ⚡ LoadingView 애니메이션 효율성 개선
- 📊 TCA ViewStore 선택적 구독 도입
...
### 케이스 2: TCACoordinator → TCAFlow 마이그레이션 프로젝트
```bash
npm start "TCACoordinator를 TCAFlow로 완전 마이그레이션해줘"
```

**자동 마이그레이션 결과:**
- 🔄 **forEachRoute → handleRoute 자동 변환**: 15개 화면 마이그레이션
- 🏗️ **@FlowCoordinator 매크로 적용**: Equatable 자동 생성으로 보일러플레이트 90% 제거
- ⚡ **TCARouter → TCAFlowRouter 변환**: 라우팅 성능 30% 개선
- 🧹 **routeWithDelaysIfUnsupported 제거**: 불필요한 코드 완전 제거
- 📱 **Swift 6 Concurrency 호환**: Sendable 준수 자동 적용

**마이그레이션 효과:**
- 코드 라인 수: **40% 감소**
- 라우팅 성능: **30% 향상**
- 컴파일 시간: **25% 단축**
- 매크로 활용으로 유지보수성: **60% 향상**

### 케이스 3: @FlowCoordinator 매크로 최적화 프로젝트
```bash
npm start "@FlowCoordinator Swift 6 호환성 최적화해줘"
```

**자동 최적화 결과:**
- 🎯 **Sendable 프로토콜 자동 적용**: Swift Concurrency 완벽 호환
- 🔧 **매크로 생성 코드 최적화**: Equatable 구현 자동화
- ⚡ **TCAFlowRouter 성능 향상**: Route 배열 관리 효율성 개선
- 📋 **GitHub 소스 기반 업데이트**: 최신 패턴 실시간 반영

### 케이스 4: WeaveDI 3.4.0 혁신 아키텍처 적용 프로젝트
```bash
npm start "WeaveDI 3.4.0 @DependencyConfiguration 혁신 패턴 적용해줜"
```

**WeaveDI 3.4.0 혁신 결과:**
- 🚀 **@DependencyConfiguration 선언적 설정**: SwiftUI ViewBuilder 패턴으로 90% 코드 감소
- 🏗️ **@Component 컴파일 타임 생성**: Needle 스타일로 10배 빠른 성능 달성
- ⚡ **AutoDIOptimizer**: 의존성 그래프 자동 분석으로 Actor hop 제거
- 🔧 **@AutoRegister 자동 등록**: 수명주기 컴파일 타임 관리로 휴먼 에러 제거
- 📱 **Swift 6 Strict Concurrency**: @DIActor 매크로로 완벽 호환

**혁신 아키텍처 효과:**
- 코드 라인 수: **90% 감소** (50줄 → 5줄)
- 컴파일 시간: **50% 단축** (9개 → 3개 모듈)
- Actor hop 최적화: **100% 자동 제거**
- 런타임 크래시: **0% (컴파일 타임 검증)**

### 케이스 5: Swinject → WeaveDI 3.4.0 완전 마이그레이션
```bash
npm start "Swinject를 WeaveDI 3.4.0 혁신 아키텍처로 마이그레이션해줘"
```

**혁신적 마이그레이션 결과:**
- 🔄 **런타임 리플렉션 → 컴파일 타임**: 앱 시작 속도 60% 향상  
- 🎯 **@Injected 타입만 해결**: `@Injected var service: Service` 단순화
- 🏗️ **UnifiedDI & ModuleFactoryManager**: 통합 아키텍처 적용
- 📋 **@DependencyGraph**: 순환 의존성 컴파일 타임 검증으로 런타임 안정성 100%
- ⚡ **TCA 타입 충돌 완전 해결**: `@Dependency`와 `@Injected` 동시 사용 가능
...
```bash
# Critical 이슈만 자동 수정
npm start "Critical 이슈만 자동 수정해줘"

# 특정 영역만 자동 최적화
npm start "SwiftUI만 자동 최적화해줘"
```

### 승인 모드 설정
- **개별 승인**: 각 수정사항마다 개별 승인 (기본값)
- **일괄 승인**: 전체 개선 계획 한 번에 승인
- **분석만**: 수정 없이 분석 및 계획만 생성
...
```bash
# 매일 아침 자동 최적화
npm start "어제 작업한 코드 최적화해줘"

# 배포 전 최종 점검  
npm start "배포 전 성능 최적화 및 자동 수정"

# 특정 기능 개발 후
npm start "QR 스캐너 기능 최적화해줘"

# TCAFlow 개발 워크플로우 (신규)
npm start "새로 추가한 @FlowCoordinator 최적화해줘"
npm start "TCACoordinator 패턴을 TCAFlow로 점진적 마이그레이션해줘"
npm start "GitHub TCAFlow 최신 업데이트 확인하고 패턴 동기화해줘"

# WeaveDI 3.4.0 혁신 개발 워크플로우 (신규)
npm start "새로 추가한 @Component를 AutoDIOptimizer로 최적화해줘"
npm start "@DependencyConfiguration 선언적 설정으로 코드 90% 단축해줘"
npm start "@DIActor 매크로로 Swift 6 Concurrency 자동 적용해줘"
npm start "UnifiedDI와 TCA @Dependency 타입 충돌 해결해줘"
npm start "@DependencyGraph로 순환 의존성 컴파일 타임 검증해줘"
```
...
- name: iOS 자동 성능 최적화
  run: |
    npm install
    npm start "자동 최적화 및 PR 생성"
```

## 📞 지원

iOS 자동 성능 최적화 시스템 사용 중 문제가 있거나 질문이 있으시면 언제든지 문의해주세요.

---

## 🎯 TCAFlow & TCACoordinator 완벽 가이드

### TCAFlow (권장) vs TCACoordinator (구식)

| 기능 | TCAFlow (신규) | TCACoordinator (구식) |
|------|-------------|------------------|
| **매크로** | `@FlowCoordinator` (Equatable 자동) | 수동 Equatable 구현 |
| **라우터** | `TCAFlowRouter` | `TCARouter` |
| **패턴** | `handleRoute` | `forEachRoute` |
| **성능** | 최적화됨 | 상대적으로 느림 |
| **Swift 6** | 완벽 호환 (Sendable) | 호환성 이슈 |
| **유지보수** | 높음 (매크로 활용) | 낮음 (보일러플레이트) |

### 🔄 자동 마이그레이션 프로세스

```mermaid
graph TD
    A[TCACoordinator 프로젝트 감지] --> B[GitHub TCAFlow 소스 확인]
    B --> C[forEachRoute 패턴 식별]
    C --> D[handleRoute 자동 변환]
    D --> E[@FlowCoordinator 매크로 적용]
    E --> F[TCAFlowRouter 업그레이드]
    F --> G[Swift 6 Sendable 적용]
    G --> H[성능 최적화 완료]
```

## 🏗️ Swift DI 아키텍처 완벽 가이드

### Swift DI 라이브러리 혁신 비교 (2024 최신)

| 기능 | WeaveDI 3.4.0 (🚀 혁신) | Swinject | Point-Free Dependencies |
|------|----------------------|----------|----------------------|
| **매크로 활용** | 🚀 @Component, @AutoRegister, @DIActor | ❌ 없음 | ❌ 없음 |
| **선언적 설정** | 🚀 @DependencyConfiguration (90% 코드 감소) | ❌ 복잡한 설정 | ⚠️ 부분적 |
| **컴파일 타임 최적화** | 🚀 AutoDIOptimizer, Actor hop 제거 | ❌ 런타임만 | ⚠️ 부분적 |
| **Swift 6 Concurrency** | 🚀 Strict Concurrency 네이티브 | ❌ 호환성 이슈 | ✅ 호환 |
| **TCA 통합** | 🚀 타입 충돌 완전 해결 | ❌ 충돌 가능 | ✅ 설계된 통합 |
| **성능** | 🚀 10배 빠름 (Needle 스타일) | ⚠️ 리플렉션 오버헤드 | ✅ 높음 |
| **코드 단순성** | 🚀 5줄로 해결 | ❌ 50+ 줄 필요 | ⚠️ 보통 |
| **순환 의존성 검증** | 🚀 @DependencyGraph 컴파일 타임 | ❌ 런타임 에러 | ❌ 런타임 에러 |

### 🔄 WeaveDI 마이그레이션 자동화 프로세스

```mermaid
graph TD
    A[DI 라이브러리 감지] --> B[Swinject/Service Locator 식별]
    B --> C[WeaveDI 3.4.0 패턴 분석]
    C --> D[@DependencyConfiguration 선언적 변환]
    D --> E[@Component Needle 스타일 적용]
    E --> F[@AutoRegister 자동 등록]
    F --> G[UnifiedDI 통합]
    G --> H[Swift 6 호환성 완료]
```

### 📚 필수 참조 링크
- **WeaveDI GitHub**: https://github.com/Roy-wonji/WeaveDI
...
- **예시 프로젝트**: Roy-wonji/WeaveDI/Example/
