---
name: fable-grade
description: "Fable-5급 시니어 산출물을 '지향'하는 작업 프로토콜 — 모델을 Fable 5로 바꾸는 게 아니라(불가능), 현재 모델(Haiku/Sonnet/Opus)이 자기 능력 상한 안에서 최대치를 내도록 규율을 강제한다. 아키텍처 설계·멀티파일 리팩터·마이그레이션·구조 재설계·근인 디버깅에 사용한다. '설계', '아키텍처', '리팩터', '마이그레이션', '구조 재설계', '모듈 분리', '왜 이렇게 동작', 'Tuist', 'xcconfig' 등에서 자동 발동. 핵심: 근거 우선(file:line 인용, 추측 금지)·규약 추출·현실 규모에 맞춘 단순화·트레이드오프 표면화·검증 가능한 마이그레이션·하드 제약 존중. 단순 1~2 step 자명 작업엔 쓰지 않는다."
---

# fable-grade — 시니어 산출물 프로토콜

이 스킬은 현재 모델(Haiku/Sonnet/Opus)의 산출물을 **Fable-5급(시니어 아키텍트) 결과에 가깝게 끌어올리는** 작업 규율이다.

> **오해 금지 — 모델 교체가 아니다.** 이 스킬은 실행 중인 모델을 Fable 5로 바꾸지 못한다(불가능). 하는 일은 **작업 절차·체크리스트를 강제**해서 지금 모델이 **자기 능력 상한 안에서 최대치**를 내게 하는 것이다. 규율로 커버되는 부분(누락 방지·근거 강제·구조화·검증)은 크게 오르지만, **순수 추론 깊이·복잡한 판단력**은 결국 모델 자체 능력에 묶인다. 주니어에게 시니어의 체크리스트를 쥐여주면 결과물이 확 좋아지지만 주니어가 시니어가 되는 건 아닌 것과 같다.

"똑똑한 모델"이 아니라 **규율**이 (도달 가능한 범위에서) 품질을 만든다. 아래 8개 품질 기둥과 실행 프로토콜을 그대로 따른다.
