...
## Fable-5급 산출 강제 (모델 등급 무관, 최우선) — `fable-grade` 스킬

Haiku·Sonnet·Opus 어느 모델로 돌든 **시니어 아키텍트(Fable-5)급 결과**를 내는 것이 기본값이다. "똑똑한 모델"이 아니라 **규율**이 품질을 만든다. 아키텍처 설계·멀티파일 리팩터·마이그레이션·구조 재설계·근인 디버깅에서는 `fable-grade` 스킬을 발동해 아래 8기둥을 체크리스트로 밟는다.
